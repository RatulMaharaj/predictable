...
class StaticRiskRates(np.ndarray):
    ...
    def project(self, term: int):
        ...
        if len(self) == term:
            ...
            return StaticRiskRates(input_array=results, label=self.label)
        elif len(self) > term:
            results = self[: term + 1]
            return StaticRiskRates(input_array=results, label=self.label)


class RiskRates(np.ndarray):
    ...
    def project(self, term: int) -> StaticRiskRates:
        """This method is used to handle the projection logic for the component.
